    ...
    public class EasyComparer
    {
        ...
        private void OutputFile(List<Wz_File> fileNew, List<Wz_File> fileOld, Wz_Type type, List<CompareDifference> diffLst, string outputDir)
        {
            ...
            try
            {
                ...
                sw.WriteLine("<tr><td>신규파일</td><td>{0}</td><td>{1}</td><td>{2}</td></tr>",
                ...
                sw.WriteLine("<tr><td>이전파일</td><td>{0}</td><td>{1}</td><td>{2}</td></tr>",
                ...
                foreach (CompareDifference diff in diffLst)
                {
                    switch (diff.DifferenceType)
                    {
                        case DifferenceType.Changed:
                            {
                                StateInfo = string.Format("{0}/{1}비교중 {2}", count[0], count[3], diff.NodeNew.FullPath);
                        ...
                        case DifferenceType.Append:
                            if (this.OutputAddedImg)
                            {
                                StateInfo = string.Format("{0}/{1}신규자료 추출중 {2}", count[1], count[4], diff.NodeNew.FullPath);
                        ...
                        case DifferenceType.Remove:
                            if (this.OutputRemovedImg)
                            {
                                StateInfo = string.Format("{0}/{1}자료 삭제중 {2}", count[2], count[5], diff.NodeOld.FullPath);
        ...
        {
            StateDetail = "img 파일 푸는 중";
            if (!imgNew.TryExtract() || !imgOld.TryExtract())
                return;
            StateDetail = "img 파일 비교중";
            ...
            StateDetail = "자료 리포트 작성중";
            sw.WriteLine("<table class=\"img\">");
            sw.WriteLine("<tr><th colspan=\"3\"><a name=\"{1}\">{0}</a> 수정:{2} 신규:{3} 제거:{4}</th></tr>", imgName, anchorName, count[0], count[1], count[2]);
