                ...
                case RenderObjectType.Mob:
                    {
                        ...
                        PrepareTextBlock(blocks, env.Fonts.TooltipContentFont, "코드:" + p.LifeID.ToString("d7"), ref current, Color.White);
                        ...
                        PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "물리/마법방어력: " + info.PADamage + " / " + info.MADamage, ref current, Color.White, ref size.X);
                ...
                case RenderObjectType.Npc:
                    {
                        ...
                        foreach (var kv in p.Actions)
                        {
                            ...
                            {
                                PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "상태: " + kv.Key, ref current, Color.White, ref size.X);
                            }
                        }
                        size.Y = current.Y;
                    }
                ...
                case RenderObjectType.Portal:
                    {
                        ...
                        Vector2 current = Vector2.Zero;
                        PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "포탈코드: " + p.PortalName, ref current, Color.White, ref size.X);
                        string pTypeName = GetPortalTypeString(p.PortalType);
                        PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "포탈유형: " + p.PortalType + (pTypeName == null ? null : (" (" + pTypeName + ")")), ref current, Color.White, ref size.X);
                        stringLinker.StringMap.TryGetValue(p.ToMap, out sr);
                        PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "이동위치: " + (sr == null ? "(null)" : sr.Name) + "(" + p.ToMap + ")", ref current, Color.White, ref size.X);
                        PrepareTextLine(blocks, env.Fonts.TooltipContentFont, "연결코드: " + p.ToName, ref current, Color.White, ref size.X);
